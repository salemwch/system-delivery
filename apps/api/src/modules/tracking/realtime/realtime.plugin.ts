...
export async function registerRealtime(
  ...
  app.get("/v1/realtime", { websocket: true }, (socket, request: FastifyRequest) => {
    void handleConnection(socket, request, gateway);
  });
}
...
async function handleConnection(
  ...
  const connection = await gateway.accept(socket, extractToken(request));
  if (connection === null) {
  ...
  socket.on("message", (data: unknown) => {
    void gateway.handleMessage(connection, String(data));
  });
  socket.on("close", () => {
