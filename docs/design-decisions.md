...
- **All three session-minting endpoints return ONE envelope** (`SessionResponse`): `/auth/login`, `/auth/mfa/challenge`, `/auth/mfa/bootstrap/confirm`. Only login carried `user`; the other two returned bare tokens, so a client that parsed login threw on the MFA path. `apps/web` reported it as invalid credentials — **no OWNER or FINANCE could sign in at all**
- **The bootstrap challenge goes in the BODY, not a Bearer header,** and the response field is `provisioningUri`, not `uri`. `apps/web` had both wrong, so privileged sign-in died before the code box ever appeared
- **The TOTP QR is rendered SERVER-side as inline SVG.** The provisioning URI contains the shared secret; drawing it in the browser would put the factor into client JavaScript. SVG markup also renders under a strict `img-src` with no `data:` allowance. The secret is still shown for manual entry — a QR that will not scan must not be the only way in
- **MFA enrolment is proven by the STORED SECRET, never by `mfa_enabled`.** The flag is set at provisioning for privileged roles long before a secret exists (migration 0023 says so itself). The login guard read the flag, so an account with it set and no secret fell through to the challenge branch and was asked for a code no authenticator could produce — **every OWNER/FINANCE/PLATFORM_ADMIN minted through `UserService.create` was born permanently locked out.** Found on a real account in the dev database. The MFA suite missed it because `seedUser` clears the flag, seeding the opposite of what production creates
- **`requiresMfa` derives from `MFA_REQUIRED_ROLES`,** never a restated list — the same second-source-of-truth bug that once dropped MERCHANT from `rolesOf`
- **Owner lockout recovery is `SEED_RESET_EMAIL` on the seed script,** and cannot be an endpoint: the reset API needs `user:manage`, which needs an OWNER session, which is exactly what is missing. It CLEARS enrolment rather than disabling MFA — "turn MFA off" is the account-takeover path 0023 warns about
...
- **NEVER refresh a rotating token during a render — it revokes the session permanently.** `cookies().set()` THROWS in a Server Component (Next allows it only in a Server Action or Route Handler), and the API rotates refresh tokens and treats a second use as theft (`revokeReason: 'REUSE_DETECTED'`, whole family). A page render therefore spent the token, failed to store the replacement, left the spent one in the cookie, and the next request got the entire family revoked. Result: 500 on every page, surviving restarts. Rotation now lives ONLY in `app/[locale]/session/refresh/route.ts`; `currentSession()` redirects there and never calls `refresh()` itself
- **A 401 from the API takes the same path as an expiry** — renew or be sent to sign in, never a 500
- **A Server Action refreshes IN PLACE; only a render redirects.** Redirecting out of an action discards whatever the user just submitted — a merchant registration came back blank with no error, looking exactly like a dead button. `canPersistCookies()` probes by re-writing the session already present (a no-op write, no token spent) — it succeeds in an action or route handler and throws in a render
- **`sticky top-0` belongs with `h-dvh` on a sidebar.** `h-dvh` alone pins it to one viewport and lets it scroll away: on a tall page the dark panel ended mid-screen with white below, and the top nav items scrolled out of sight while the user was still on the page — which reads as "the menu items disappeared", not as a scroll position
- **Phone input is NORMALISED in the UI, not merely validated.** Nobody in Tunisia types `+21624201314`; they type `24201314`. `toE164()` accepts the national form, `00` prefix and any separators, and refuses a wrong-length national number rather than mangling it. The API stays strict — one canonical format in the database
- **`return redirectToLogin()`, not `await redirectToLogin()`.** TypeScript does not treat statements after an awaited `Promise<never>` as unreachable, so the value stays nullable
- **`next` on the refresh route is attacker-controlled** — `safeReturnPath` rejects absolute, protocol-relative, `/\`-smuggled and control-character paths. An open redirect that only fires for signed-in staff is precisely the one worth phishing
- **A `datetime-local` value has NO timezone — read it in the TENANT's zone, never the server's.** `new Date("2026-08-07T09:00")` parses in the server's zone: correct only while the server happens to run in Africa/Tunis, and silently an hour out the day it moves to a UTC container. `zonedToUtcIso()` resolves the offset against the IANA zone ON THAT DATE, so DST is right too. Two passes: the first offset is measured at the wrong instant and lands an hour out across a DST boundary
- **Each workflow button renders on exactly the status its transition starts from** — accept on REQUESTED, claim on ACCEPTED. A button anywhere else is an offer the API refuses, and the user cannot tell that from a bug
- **Surface the API's error code, never a blanket "request failed".** Every form rendered one message for any non-field error, so `MERCHANT_CODE_TAKEN` — a one-word fix — was indistinguishable from the database being down; the user retried the same code and got the same nothing. `apiErrorMessage()` maps known codes and keeps an UNMAPPED code visible in the text, so a missing translation is diagnosable rather than swallowed. `FIELD_FOR_ERROR` puts the message under the input that caused it
- **CSP must carry a NONCE and cannot be a static header.** `script-src 'self'` in `next.config.ts` blocked every inline script Next uses to hydrate React. Nothing errored visibly: the server HTML was perfect and the client runtime never started, so **a form's submit button did nothing**. The rendered form is `action=""` — Next binds the Server Action client-side — so no JS means no submit at all. Built per request in `proxy.ts` now, set on the REQUEST header too (that is how Next finds the nonce to stamp its scripts) and on the response. All three Next apps had it
- **`'unsafe-eval'` is development-only** (Turbopack HMR); **Google Font hosts are development-only** (the Next error overlay fetches its own webfonts, and blocking them buries real errors under CSP noise); **`upgrade-insecure-requests` is production-only** (it would break http://localhost)
