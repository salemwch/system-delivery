...
- **A layout `redirect()` does NOT protect its pages — middleware does.** Next renders layout and page CONCURRENTLY: the page's first `apiFetch` hit `readSession()` → null → `NotAuthenticatedError` and returned a 500 stack trace before the layout's redirect could land. `src/middleware.ts` runs before rendering, so there is no race. It checks cookie PRESENCE only — the seal is AES-256-GCM and only the server opens it, so a forged cookie passes middleware and is rejected by `readSession()`, which stays the authority
