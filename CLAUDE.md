...
**Everything green:** `pnpm build` → ok · `pnpm test` → 1188/1188 (1029 api + 39 track + 33 merchant + 87 web) · MFA sign-in verified end to end · `pnpm lint` → 0 · `pnpm lint:rules` → 6/6 · `pnpm knip` → 0 · `pnpm sast` → 0 (355 targets, all four apps tracked and covered).
...
| Migrations     | 0000–0035 applied. Forward-only, checksum-locked, immutable. See individual migration files for DDL details                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- **⚠️ A FOREIGN KEY CHECK BYPASSES RLS.** Postgres runs referential integrity with privileges that see the whole table, so `REFERENCES merchants (id)` accepts another tenant's merchant id. The insert succeeds and the row is invisible to both tenants. The fix is a composite key: `UNIQUE (tenant_id, id)` on the parent, `FOREIGN KEY (tenant_id, child_id) REFERENCES parent (tenant_id, id)` on the child — `MATCH SIMPLE` skips the check when the child column is NULL, so nullable FKs still work. Done for `notes` (0035); **46 older single-column FKs still have the hole** (hubs 10, addresses 9, users 6, merchants 5, shipments 4, drivers 3, vehicles 3, routes 2, zones 2, recipients 2)
