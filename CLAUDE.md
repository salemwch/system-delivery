...
**Everything green:** `pnpm verify` → exit 0 · `pnpm test` → 107/107 · `pnpm build` → ok · `pnpm sast` → 0 findings.
...
| Migrations          | `0000` tenants+tenant_features, `0001` tenants-registry RLS fix, `0002` identity, `0003` outbox, `0004` outbox relay (`next_attempt_at` + `dp_relay` grant/policy), `0005` directory (merchants, recipients, addresses; PostGIS geography + RLS). Runner: `apps/api/src/shared/database/migrator.ts` — forward-only, checksum-locked (**applied migrations are immutable**)                                                                |
...
1. **`shipment` module** — the core aggregate: immutable `shipment_events`, status projection, state machine, POD. Layer 2. Composes `directory` (`RecipientService.findByPhone`/`create`, `AddressService.resolve`, `MerchantService.getById`) + snapshots recipient name/phone onto the shipment. The SAST rule `no-direct-shipment-status-write` starts firing here.
2. Then `network` (hubs/zones/geofences), `fleet` (drivers/vehicles/shifts), `dispatch`.
