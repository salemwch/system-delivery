...
**Everything green:** `pnpm build` → ok · `pnpm test` → 1296/1296 (1137 api + 39 track + 33 merchant + 87 web) · MFA sign-in verified end to end · `pnpm lint` → 0 · `pnpm lint:rules` → 6/6 · `pnpm knip` → 0 · `pnpm sast` → 0 (355 targets, all four apps tracked and covered).
...
| Migrations     | 0000–0040 applied. Forward-only, checksum-locked, immutable. See individual migration files for DDL details                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| RLS            | Data tables: ENABLE+FORCE. `tenants` registry: ENABLE only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Tenant FKs     | **All 55 tenant-scoped foreign keys are composite `(tenant_id, id)` (0035/0036).** RLS alone does not isolate a reference — an FK check runs outside it. 7 tests, one of which fails if a single-column FK ever reappears                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `directory`    | MerchantService, RecipientService, AddressService. **`merchants.account_manager_id` — the COMMERCIAL who owns the account; set from ambient context on create, moved only via `PUT /v1/merchants/:id/account-manager` (`merchant:assign_manager`, audited both sides).** **Geocoding is real: self-hosted Nominatim behind the port, chained so low-confidence results fall through to a commercial provider. `GEOCODER=manual` by default.** **MerchantApplicationService — nouveaux clients: an application is its OWN entity, never a status on `merchants`, and approving one CREATES the merchant. `POST /v1/merchant-applications/public/:tenantSlug` is unauthenticated; a duplicate phone returns 202 like everything else so it cannot be used to test whether a number is known, plus a 30/hour per-tenant cap that staff-logged leads bypass. `merchant:decide_application` is separate from `merchant:create`; a COMMERCIAL who approves becomes the account manager (I25).** 60 tests                                                                                                                                                                                          |
| `network`      | HubService (resolveForAddress via ST_Covers/KNN), ZoneService, GeofenceService (pure evaluate). **CityService — villes: per-tenant coverage list and tariff (delivery/return fee in minor units, lead time, gouvernorat, optional zone link). `normaliseCityKey` folds French accents and Arabic letter forms (أإآٱ→ا, ى→ي, ة→ه, tashkeel, tatweel) into `search_keys`, GIN-indexed; bulk `POST /v1/cities/resolve` prices a whole CSV in one query. Collision-guarded against active rows, retired never deleted, fee changes audited.** 57 tests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `fleet`        | VehicleService, DriverService (PII encrypted), ShiftService (privacy gate). 19 tests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `shipment`     | ShipmentEventService.applyTo = single status writer. ShipmentService commands (create, pickup, deliver, fail, initiateReturn, **completeReturn**, cancel) — the full RTO lifecycle (2.8). AddressBookService — the merchant's own recipients, projected from RLS-narrowed shipments (resolves RM-R1). **ShipmentAmendmentService — modification colis: `shipment:update` asks, `shipment:amend_approve` answers, and holding the latter applies your own request on the spot. One PENDING amendment per parcel (partial unique index). Terminal parcels refuse; COD refuses once COLLECTED. Applying moves `cod_amount_minor` and `cod_status` TOGETHER, re-resolves the recipient when the phone changes, re-geocodes outside the transaction, and snapshots only the touched fields into `previous`. Publishes `shipment.amended` carrying the NEW phone — and deliberately no address and no amount, because the SMS reaches whatever number is on file. Every read joins the parcel's currency, so no endpoint can return an amount without one.** 61 tests                                                                                                                                                                                                                                                                                                                                     |
...
| `notification` | EventStreamConsumer + NotificationEventHandler (10 events: customer SMS, merchant SMS, driver PUSH) + NotificationService. **HttpSmsProvider** (vendor-neutral, circuit breaker) · **FcmPushProvider** (no firebase-admin) · **ChannelRoutingProvider** · **TemplateService** — per-tenant per-locale overrides, placeholder validation, SMS segment estimation. 29 tests                                                                                                                                                                                                                                                                           |
...
| `support`      | **Support — SupportService.** The merchant/back-office conversation: a ticket is a QUESTION, a complaint is a CLAIM, and they are separate tables for the same reason. Threaded messages with a `visibility` column — **an INTERNAL note is hidden from a merchant login by RLS, not by a query filter**, so it cannot leak by forgetting a WHERE clause. Status moves with the reply (courier → PENDING_MERCHANT, merchant → OPEN); an internal note moves nothing. Gapless-ish `S-2026-00001` reference from a row-locked counter, the same mechanism as invoices. Layer 3, imports nothing above layer 0. 26 tests |
| `note`         | **Remarques — NoteService.** Staff remarks on a shipment, merchant or driver. Body immutable once written (trigger); a correction is a new note. Subject is three exclusive **composite** FKs (`num_nonnulls = 1`), never a polymorphic id — so the database proves the subject exists and is in the same tenant, and this module (layer 3) imports nothing above layer 0. Pin, resolve/reopen, open-queue count. MERCHANT and COMMERCIAL hold neither permission by design. 22 tests |
...
| `finance`      | **Inc5 — les dépenses:** expense categories + expenses with a two-step DRAFT→APPROVED flow, and **approving posts a REAL double-entry transaction — DEBIT EXPENSE, CREDIT the source**. Cash paid from a hub box credits HUB_CASH, so the balance `cashInField` reconciles against goes down; without it the hub reads as short. New `EXPENSE` account type (DEBIT-normal) and entry type. Partial unique index on the supplier reference stops the same fuel bill being entered twice. The spend report totals APPROVED rows only, grouped by currency, `sum()` parsed as bigint. 24 tests |
| `finance`      | **Inc1:** currencies + double-entry ledger (zero-sum DEFERRABLE trigger). **Inc2:** remittance (submit/confirm/dispute). **Inc3:** settlement (draft→approve→pay with separation-of-duties) + reconciliation (cashInField, dailyReconciliation). **Inc4 — factures et avoirs:** gapless per-tenant/kind/year numbering (`FA-2026-00001`, `AV-…`) from a row-locked counter, NOT `nextval()`; immutable once issued (DB triggers, ISSUED→PAID only); credit notes as the sole correction path; per-tenant TVA in basis points + timbre fiscal; party snapshot frozen at issue; printable A4 HTML in ar/fr/en. `invoice:draft` and `invoice:issue` are separate permissions. 89 tests                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `apps/merchant`| **Merchant portal** — Next.js 16 + React 19, server-rendered with client form components. AES-256-GCM sealed session cookie (httpOnly, token never in browser JS). Dashboard, shipment CRUD, address book, **public `/register` account application (unauthenticated, allowed through `proxy.ts` alongside `/login`; the form is REPLACED by a confirmation on success — the applicant has no other feedback)**, ar/fr/en RTL. 17 tests                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `apps/web`     | **Dispatcher board + admin console** — Next.js 16 + React 19, server-rendered, 21 routes. Merchant write surface: register, detail + per-merchant stats, mint portal login (password shown once), set/replace pickup address, reassign account manager, request a pickup. Printable dockets (bon de livraison/envoi/retour), CSV bulk import per merchant, Settings → SMS templates, zones, **villes (tariff editor, search, retire/restore; the CSV import refuses a destination the courier does not serve, skipped when no coverage list is configured)**. Pickups: accept on REQUESTED, claim on ACCEPTED — each button on exactly the status its transition starts from. **The commercial's loop is UI-complete: register → address → portal login → request → accept → claim.** AES-256-GCM sealed session (separate secret from merchant). Role-gated sidebar (OWNER, DISPATCHER, HUB_OPERATOR, FINANCE, PLATFORM_ADMIN). COD amounts redacted for DISPATCHER. MFA login flow (challenge + bootstrap enrolment). Dashboard, shipments, dispatch, fleet, network, merchants, pickups, custody, finance (ledger/remittances/settlements/**invoices**), complaints, users, audit. **Invoicing UI: list with status filters, draft with a growing line table, issue/pay/cancel/credit-note each rendered only on the status its transition starts from, printable document proxied through a Route Handler, billing settings.** **Modification Colis: a pending/applied/rejected queue, plus a panel on every parcel that shows the change history as `field: old → new` and hides the request form while one is already open.** **Nouveaux clients: pending/approved/rejected queue, oldest-first, approve (asks for the code) or reject (asks for a reason), plus a collapsed form for a commercial to log a lead.** **Les dépenses: a DRAFT approval queue with the month's spend per category above it, a record form whose "paid from" choice drives whether a hub cash balance moves, and a category form that opens automatically on a fresh tenant because the expense form cannot be submitted without one.** **Support: a back-office queue with OPEN and PENDING_MERCHANT as SEPARATE tabs (a ticket waiting on the merchant is not the courier's backlog), a thread view where internal notes are amber-labelled, and a collapsed form to open one on a merchant's behalf.** **Remarques: an open/resolved queue as a sidebar section, plus an inline panel on every shipment and merchant page — write, pin, resolve; no edit control anywhere, because the body is frozen in the database.** ar/fr/en RTL. 35 tests |
...
- **An invoice number comes from a row-locked counter, never a sequence.** `nextval()` does not roll back, and a gap in a tax series reads to an auditor as a destroyed invoice. `SELECT … FOR UPDATE` on `invoice_sequences` is load-bearing — without it two concurrent issues produce the same number. ⚠️ A concurrency test on a COLD sequence proves nothing: the `INSERT … ON CONFLICT DO NOTHING` that creates the counter row serialises the burst by itself. Issue one invoice first
- **Decimal → minor units by STRING arithmetic, never `Math.round(x * 10 ** n)`.** `4.005 * 1000` is `4004.999999999999`; that becomes a 4.004 TND unit price on a signed document. `toMinorUnits()` in `apps/web/src/lib/minor-units.ts`
- **⚠️ A FOREIGN KEY CHECK BYPASSES RLS.** Postgres runs referential integrity with privileges that see the whole table, so `REFERENCES merchants (id)` accepts another tenant's merchant id — the insert succeeds and the row is invisible to both tenants. **Every tenant-scoped FK is now COMPOSITE** (0035 for `notes`, 0036 for the other 54): `UNIQUE (tenant_id, id)` on the parent, `FOREIGN KEY (tenant_id, child_id) REFERENCES parent (tenant_id, id)` on the child. Two rules when adding one: (a) `MATCH SIMPLE` skips the check when any child column is NULL, so nullable FKs need no special handling; (b) **`ON DELETE SET NULL` must name its column** — `SET NULL (child_id)` — or it nulls `tenant_id` too, which is NOT NULL, and the delete fails at runtime long after review. `tenant-foreign-keys.spec.ts` fails if any future migration reintroduces a single-column FK
- **An unauthenticated endpoint must answer the same way whether or not it recognises you.** The public application intake returns 202 for a duplicate phone exactly as for a new one — a differing response is a way to test whether a number is known to the courier. The partial unique index does the deduplication and the service swallows *only* that one constraint
- **A city name is matched on a NORMALISED key, never the raw string.** `normaliseCityKey` NFD-decomposes first so one `\p{M}` strip removes both French accents and Arabic tashkeel, then folds ٱ→ا, ى→ي, ة→ه. Deriving it in SQL is not an option: `unaccent` is STABLE, so it cannot appear in a generated column or an index. `search_keys` is maintained by `searchKeysFor` on every write — nothing else may set it, or the stored keys and the lookup drift apart and a merchant is billed the wrong tariff with no error anywhere
- **An array parameter needs `ARRAY[$1,$2]::text[]`, not `${jsArray}`.** A JS array bound as one parameter has no inferred element type, and `text[] && unknown` fails at plan time with "operator does not exist"
- **VAT on the SUBTOTAL, not per line**, rounded half away from zero; the timbre fiscal is added AFTER tax and is never itself taxed
