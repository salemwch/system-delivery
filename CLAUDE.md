...
**Everything green:** `pnpm verify` → exit 0 · `pnpm test` → 91/91 · `pnpm build` → ok · `pnpm sast` → 0 findings.
...
| Migrations         | `0000` tenants+tenant_features, `0001` tenants-registry RLS fix, `0002` identity, `0003` outbox, `0004` outbox relay (`next_attempt_at` + `dp_relay` grant/policy). Runner: `apps/api/src/shared/database/migrator.ts` — forward-only, checksum-locked (**applied migrations are immutable**)                                                                                                  |
...
1. **`directory` module** — `merchants`, `recipients` (unique on `(tenant, phone)`, snapshot on shipment), `addresses` (+ geocode confidence). Layer 1. **First business module with CRUD controllers.**
2. **`shipment` module** — the core aggregate: immutable `shipment_events`, status projection, state machine, POD. Layer 2. The SAST rule `no-direct-shipment-status-write` starts firing here.
3. Then `network` (hubs/zones/geofences), `fleet` (drivers/vehicles/shifts), `dispatch`.
