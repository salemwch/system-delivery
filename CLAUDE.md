...
**Everything green:** `pnpm verify` → exit 0 · `pnpm test` → 123/123 · `pnpm build` → ok · `pnpm sast` → 0 findings.
...
1. **`network`** (hubs/zones/geofences), then **`fleet`** (drivers/vehicles/shifts), then **`dispatch`**. Once `network`/`fleet` land, the shipment events those contexts OWN (`assigned` from dispatch; `arrived_at_hub`/`loaded`/`departed` from hub) move from the generic `ShipmentService.recordEvent` seam to their own commands with richer payloads (driverId/vehicleId, hubCode, manifestId). The state machine and append writer already support them.
