...
**Everything green:** `pnpm build` → ok · `pnpm test` → 735/735 · `pnpm lint` → 0 · `pnpm knip` → 0 · `pnpm sast` → 0.
...
| `platform`     | OutboxService, FeatureService, TenantService, OutboxRelayService, ValkeyStreamEventPublisher, EventPublisher port. **AuditService** — append-only trail, monthly partitions, secret redaction, `GET /v1/audit` (26 tests). **OperatingConfigService** — per-tenant failure taxonomy, working hours, holidays, SLA templates, and the working-calendar arithmetic that schedules re-attempts (28 tests)                                                                                                                                                                                                                                             |
